...
void Misc::watermark(const Memory& memory) noexcept
{
    ...
    ImGui::Text("p2wcs | %d fps | %d ms", frameRate != 0.0f ? static_cast<int>(1 / frameRate) : 0, GameData::getNetOutgoingLatency());
    ImGui::End();
}
...
void Misc::voteRevealer(const ClientInterfaces& clientInterfaces, const OtherInterfaces& interfaces, const Memory& memory, const GameEvent& event) noexcept
{
    ...
    memory.clientMode->getHudChat()->printf(0, " \x0C\u2022p2wcs\u2022 %c%s\x01 voted %c%s\x01", isLocal ? '\x01' : color, isLocal ? "You" : entity.getPlayerName(interfaces, memory).c_str(), color, votedYes ? "Yes" : "No");
}
...
void Misc::onVoteStart(const ClientInterfaces& clientInterfaces, const OtherInterfaces& interfaces, const Memory& memory, const void* data, int size) noexcept
{
    ...
    memory.clientMode->getHudChat()->printf(0, " \x0C\u2022p2wcs\u2022 %c%s\x01 call vote (\x06%s\x01)", isLocal ? '\x01' : '\x06', isLocal ? "You" : entity.getPlayerName(interfaces, memory).c_str(), voteName(voteType));
}

void Misc::onVotePass(const Memory& memory) noexcept
{
    if (miscConfig.revealVotes)
        memory.clientMode->getHudChat()->printf(0, " \x0C\u2022p2wcs\u2022\x01 Vote\x06 PASSED");
}

void Misc::onVoteFailed(const Memory& memory) noexcept
{
    if (miscConfig.revealVotes)
        memory.clientMode->getHudChat()->printf(0, " \x0C\u2022p2wcs\u2022\x01 Vote\x07 FAILED");
}
